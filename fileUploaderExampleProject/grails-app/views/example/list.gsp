			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="firstName" title="${message(code: 'example.firstName.label', default: 'First Name')}" />
					
						<g:sortableColumn property="lastName" title="${message(code: 'example.lastName.label', default: 'Last Name')}" />
				...
				<g:each in="${exampleInstanceList}" status="i" var="exampleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${exampleInstance.id}">${fieldValue(bean: exampleInstance, field: "firstName")}</g:link></td>
					
						<td>${fieldValue(bean: exampleInstance, field: "lastName")}</td>
